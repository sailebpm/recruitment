<template>
    <div>
        ...
        <v-card-text class="pa-5">
            <v-container fluid>
              <v-data-table :loading="loading" :headers="table.headers" :items="applicant_details_user" :items-per-page="11" disable-sort hide-default-footer>
                <template v-slot:[`item.upload`]="{ item }">
                  <div v-if="item.supplemental_details.additional_info !== '' && item.status < 1 || item.status == 3" class="d-flex justify-center mt-5">
                    <v-file-input
                      v-model="item.file"
                      :label="'SELECT ' + item.supplemental_details.title" 
                  ...
                  <div v-else-if="item.supplemental_details.additional_info !== '' && item.attachment != null" class="d-flex justify-center mt-5">
                    <p class="blue--text mdi mdi-download" style="cursor: pointer;" @click="downloadFile(item)">Download</p>
                  </div>
                </template>
                <template v-slot:[`item.status`]="{ item }">
                    <div  v-if="item.supplemental_details.additional_info !== ''">
            ...
            headers: [
              {
                ...
                align: "center",
                text: "Title",
                value: "supplemental_details.title",
              },
              {
                class: "black--text",
                align: "center",
                text: "Requirements",
                value: "supplemental_details.additional_info",
              },
              {
                class: "black--text",
                width: 400,
