<template>
    <div>
        ...
        <v-card-text class="pa-5">
            <v-container fluid>
              <v-data-table :loading="loading" :headers="table.headers" :items="applicant_requirements_user" :items-per-page="10" disable-sort hide-default-footer>
                <template v-slot:[`item.upload`]="{ item }">
                  <div v-if="item.supplemental_requirements.additional_info !== '' && item.status < 1 || item.status == 3" class="d-flex justify-center mt-5">
                    <v-file-input
                      v-model="item.file"
                      :label="'SELECT ' + item.supplemental_requirements.title.substring(2)" 
                  ...
                  <div v-else-if="item.supplemental_requirements.additional_info !== '' && item.attachment != null" class="d-flex justify-center mt-5">

                    <p class="blue--text mdi mdi-download" style="cursor: pointer;" @click="downloadFile(item)">Download</p>
                  </div>
                </template>
                <template v-slot:[`item.status`]="{ item }">
                    <div v-if="item.supplemental_requirements.additional_info !== ''">
            ...
            headers: [
              {
                ...
                align: "center",
                text: "Title",
                value: "supplemental_requirements.title",
              },
              {
                class: "black--text",
                align: "center",
                text: "Requirements",
                value: "supplemental_requirements.additional_info",
              },
              {
                class: "black--text",
                width: 400,
      ...
      methods: {
       ...
       submitFiles() {
        let formData = new FormData();
          this.applicant_requirements_user.forEach((item) => {
            if (item.file) {
              formData.append("attachment[]", item.file);
              formData.append("id[]", item.id);
              formData.append("applicant_req_id[]", item.applicant_req_id);
              formData.append("entityName", "SupplementaryRequirements");
...
/* Styling for when the <p> element is hovered over */
.download:hover {
